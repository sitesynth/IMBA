...
export default async function PromoLandingPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  async function claimPromo(_formData: FormData) {
    ...
    try {
      ...
      if (e instanceof ApiError) errorMsg = e.message
    }
    revalidatePath('/dashboard')
    if (errorMsg) {
      redirect('/dashboard?promo_error=' + encodeURIComponent(errorMsg))
    } else {
      ...
      <div className="w-full max-w-sm fade-up">
        <div className="text-center mb-7">
          ...
          <h1 className="display text-4xl mb-2" style={{ lineHeight: 1.05 }}>
            Твой промокод<br />
            <span style={{ color: 'var(--yellow)', WebkitTextStroke: '1.5px var(--ink)' }}>активирован!</span>
          </h1>
